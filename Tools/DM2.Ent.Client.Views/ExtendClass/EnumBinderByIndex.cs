// <copyright file="EnumBinderByIndex.cs" company="BancLogix">
...
namespace DM2.Ent.Client.Views.ExtendClass
{
    ...
    /// 枚举绑定帮助类
    /// </summary>
    public class EnumBinderByIndex
    {
        #region Path
        ...
        public static readonly DependencyProperty PathProperty = DependencyProperty.RegisterAttached(
            "Path", typeof(string), typeof(EnumBinderByIndex), new PropertyMetadata(OnPathPropertyValueChanged));
        ...
        public static void SetPath(DependencyObject obj, string value)
        {
            ...
            else if (content.Length == 3)
            {
                ...
                var splits = content[2].Split(',');
                if (splits.Any())
                {
                    foreach (var split in splits)
                    {
                        indexs.Add(Convert.ToInt16(split));
                    }
                }
            }
            else
            ...
            for (int i = 0; i < list.Length; i++)
            {
                ...
                if (indexs.Any() && indexs.Any(o => o == i) == false)
                {
                    continue;
                }
        ...
        public static readonly DependencyProperty PathWithAllProperty = DependencyProperty.RegisterAttached(
            "PathWithAll", typeof(string), typeof(EnumBinderByIndex), new PropertyMetadata(OnPathWithAllPropertyValueChanged));
